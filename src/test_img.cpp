    ...
    while(cap.isOpened())
    {
        ...
        if(faces_info.size()>=1)
        {
            ...
            else
            {
                ...
                putText(frame, person_name, cv::Point(15, 80), cv::FONT_HERSHEY_SIMPLEX,1.5, cv::Scalar(255, 255, 0),2);  

                liveface="True face";
                cv::rectangle(frame, Point(large_box.x1*ratio_x, large_box.y1*ratio_y), Point(large_box.x2*ratio_x,large_box.y2*ratio_y), cv::Scalar(0, 255, 0), 2);
                
            }

            cout<<liveface<<"\n";

            putText(frame, liveface, cv::Point(15, 40), cv::FONT_HERSHEY_SIMPLEX,1.5, cv::Scalar(255, 255, 0),2);  
